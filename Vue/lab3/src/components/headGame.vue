    ...
    <div class="row no-gutters">
      ...
      <div class="col-md-4">
        <div class="card-body">
          ...
          <p class="card-text">
            {{
              gameStore.randomGame.genre &&
              gameStore.randomGame.genre.toUpperCase()
            }}
          </p>
          <p class="card-text">{{ gameStore.randomGame.short_description }}</p>
          <p class="card-text">
            <small class="text-muted"
              >Release Date: {{ gameStore.randomGame.release_date }}</small
            >
          </p>
          <button
            @click="onGetDetails(gameStore.randomGame.id)"
